	...
	public partial struct CommandBuilder : IDisposable {
		...
		public void WireCapsule (float3 position, float3 direction, float length, float radius) {
			...
			if (radius <= 0) {
				...
				var tangent = math.cross(direction, new float3(1, 1, 0));

				if (math.all(tangent == float3.zero)) tangent = math.cross(direction, new float3(-1, 1, 0));
		...
		private static float3 calculateTangent (float3 normal) {
			var tangent = math.cross(new float3(0.4915391f, 0.6553855f, 0.5734623f), normal);
